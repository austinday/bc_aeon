#!/bin/bash
# =============================================================================
# Start llama.cpp server for Qwen3.5-27B-UD-Q8_K_XL.gguf on SINGLE GPU
# =============================================================================
set -e
...
# Need the file
MODEL_FILE=$(cd "${MODELS_DIR}" 2>/dev/null && find . -name "*.gguf" | grep -i "Q8_K_XL" | sort | head -1 | sed 's|^\./||')
if [ -z "$MODEL_FILE" ]; then
    echo "[Qwen3.5-27B-Q8] ERROR: No .gguf files matching Q8_K_XL found in ${MODELS_DIR}"
    exit 1
fi
